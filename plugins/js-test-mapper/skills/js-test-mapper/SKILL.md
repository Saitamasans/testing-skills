...
- 仅在用户明确授权的测试环境和账号范围内工作；认证信息只用于用户指定站点，不写入文件、日志或报告。
- 只做浏览器可见的安全只读导航和证据整理；不执行新增、编辑、删除、审核、发布、支付或其他状态变更动作。
- 不主动构造、重放、探测或 fuzz 业务 API；页面自然产生的请求只观察并如实记录。

## 最终自检

- 已说明授权范围、当前账号和实际覆盖边界。
- 已区分页面观察、脚本证据、推断和未验证项，没有把页面描述冒充完整脚本分析。
- 已确认没有保存密码、Cookie、Token 或 Authorization，也没有执行业务写操作。
...
拿不到关键 JS 时仍交付有价值的受限分析，但开头明确限制，不能把页面描述冒充脚本证据分析完成。候选验收必须在真实站点证明“JS 证据 → 业务规则/调用关系 → 测试关注点”，以及自主安全探索和三种输出；结构校验通过不替代端到端验收。

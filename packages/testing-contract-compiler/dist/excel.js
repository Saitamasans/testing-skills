...
const FIELDS = ["case_id", "module", "title", "feature", "precondition", "steps", "expected", "priority", "actual_result", "execution_result"];
...
function normalized(text) { return text.replace(/\s+/g, "").toLocaleLowerCase(); }
function mappingFor(headers, explicit) {
    if (explicit)
        return Object.fromEntries(FIELDS.map((field) => [field, explicit[field] ?? null]));
...
async function inspectLoadedWorkbook(workbook, explicitMapping) {
    ...
    let selectedHeaders = [];
    let selectedMapping = {};
    let format = "non_standard";
    for (const sheet of workbook.worksheets) {
        if (sheet.rowCount < 2)
            continue;
        const headers = sheet.getRow(1).values instanceof Array
            ? sheet.getRow(1).values.slice(1).map((item) => String(item ?? "").trim())
            : [];
        ...
        if (!indexes.case_id || !indexes.title)
            continue;
        for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber += 1) {
            ...
            if (!caseId)
                continue;
            const get = (field) => indexes[field] ? value(row.getCell(indexes[field])) : "";
    ...
    if (duplicate)
        throw new Error(`duplicate_case_id: ${duplicate}`);
...
export async function inspectWorkbook(input, explicitMapping) {
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile(input);
    return inspectLoadedWorkbook(workbook, explicitMapping);
}
export async function inspectWorkbookBytes(bytes, explicitMapping) {
    const workbook = new ExcelJS.Workbook();

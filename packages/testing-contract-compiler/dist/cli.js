...
program.command("compile").requiredOption("--input <path>").requiredOption("--output <path>")
    ...
    .option("--requirement <path>", "optional requirement document; repeatable", (value, previous) => [...previous, value], [])
    ...
    const project = projectConfig ? JSON.parse(await readFile(projectConfig, "utf8")) : undefined;
    const fieldMapping = mapping ? JSON.parse(await readFile(mapping, "utf8")) : project?.field_mapping;
    const contractOverrides = overrides ? JSON.parse(await readFile(overrides, "utf8")) : project?.overrides;

...
function assertNoSecrets(value, key = "root") {
    if (Array.isArray(value))
        return value.forEach((item, index) => assertNoSecrets(item, `${key}[${index}]`));
    ...
    for (const [childKey, child] of Object.entries(value)) {
        ...
        if (sensitive && !reference && child !== null && child !== "")
            throw new Error(`secret_value_forbidden: ${key}.${childKey}`);
        if (reference && typeof child === "string" && /_env$/i.test(normalized) && !/^[A-Z][A-Z0-9_]*$/.test(child))
            throw new Error(`secret_value_forbidden: ${key}.${childKey}`);
        assertNoSecrets(child, `${key}.${childKey}`);
    }
}
function assertAcyclic(cases) {
...
function assertConsistency(cases) {
    const ids = new Set(cases.map((item) => item.case_id));
    const exclusiveLocks = new Map();
    for (const item of cases) {
        for (const dependency of item.dependencies)
            if (!ids.has(dependency))
                throw new Error(`dependency_unknown: ${item.case_id}:${dependency}`);
        const grouped = item.isolation_scope === "flow_group";
        if (grouped !== (typeof item.flow_group === "string" && item.flow_group.length > 0))
            throw new Error(`flow_group_inconsistent: ${item.case_id}`);
        for (const lock of item.resource_locks) {
            if (!lock || typeof lock !== "object")
                throw new Error(`resource_lock_invalid: ${item.case_id}`);
            const value = lock;
            if (typeof value.resource !== "string" || !value.resource || !["shared", "exclusive"].includes(String(value.mode)))
                throw new Error(`resource_lock_invalid: ${item.case_id}`);
            if (value.mode !== "exclusive")
                continue;
            const owner = exclusiveLocks.get(value.resource);
            if (owner && !item.dependencies.includes(owner))
                throw new Error(`resource_lock_conflict: ${owner}:${item.case_id}:${value.resource}`);
            exclusiveLocks.set(value.resource, item.case_id);
        }
    }
}
export function buildContract(sourceCases, overrides = {}) {
    const cases = sourceCases.map((source) => {
        const unresolved = [];
        if (!source.precondition)
            unresolved.push({ field: "start_state", reason: "missing_precondition" });
        if (!source.steps)
            unresolved.push({ field: "actions", reason: "missing_steps" });
        if (!source.expected)
            unresolved.push({ field: "assertions", reason: "missing_expected" });
        if (!["P0", "P1", "P2"].includes(source.priority))
            unresolved.push({ field: "priority", reason: "invalid_priority" });
        const base = {
    ...
    assertAcyclic(cases);
    return { contract_version: "1.0.0", cases };
}

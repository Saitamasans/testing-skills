...
function isReferenceKey(key: string): boolean {
  const tokens = tokenizeKey(key);
  return tokens.some((token) => ["env", "ref", "reference", "credential"].includes(token));
}
  ...
  const visit = (item: unknown): void => {
    ...
    if (item && typeof item === "object") {
      for (const [key, child] of Object.entries(item as Record<string, unknown>)) {
        if (isSensitiveKey(key) && !isReferenceKey(key) && child !== null && child !== "" && child !== false) throw new Error("secret_value_forbidden");
        visit(child);
      }
    }
  };

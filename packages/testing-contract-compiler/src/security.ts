...
function referenceSuffix(key: string): "env" | "ref" | "reference" | null {
  const tokens = tokenizeKey(key);
...
function isSafeSensitiveValue(key: string, value: unknown): boolean {
  if (value === null || value === "" || value === false || isStructuredReference(value)) return true;
  const suffix = referenceSuffix(key);
  if (suffix === "env") return typeof value === "string" && /^[A-Z][A-Z0-9_]*$/.test(value);
  ...
  const visit = (item: unknown): void => {
    ...
    if (item && typeof item === "object") {
      for (const [key, child] of Object.entries(item as Record<string, unknown>)) {
        if (isSensitiveKey(key) && !isSafeSensitiveValue(key, child)) throw new Error("secret_value_forbidden");
        visit(child);
      }
    }
  };
  if (typeof value === "string" && /^[\s\r\n]*[\[{]/.test(value)) {
    try { visit(JSON.parse(value)); } catch (error) {

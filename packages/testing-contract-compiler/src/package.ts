...
function validateZipStructure(bytes: Buffer): ZipEntryMetadata[] {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (localNameEnd + localExtraLength > bytes.length) throw new Error("zip_local_header_invalid");
...
export async function validatePackage(packagePath: string): Promise<ValidationResult> {
  ...
  try {
    ...
    for (const item of metadata) {
      ...
      if (!entry) throw new Error("zip_entry_missing_after_load");
      const entryBytes = Buffer.from(await entry.async("uint8array"));
      if (entryBytes.length !== item.uncompressedSize) throw new Error("zip_declared_size_mismatch");
    ...
    manifest = JSON.parse(await manifestEntry.async("string")) as PackageManifest;
    ...
    for (const sourcePath of manifest.source_files ?? []) {
      ...
      if (sha256(await entry.async("uint8array")) !== manifest.source_sha256[sourceName]) errors.push("source_sha_mismatch");
    }
    for (const name of manifest.internal_files ?? []) {
      const entry = zip.file(name);
      if (!entry) { errors.push("internal_file_missing"); continue; }
      if (sha256(await entry.async("uint8array")) !== manifest.internal_file_sha256[name]) errors.push("internal_sha_mismatch");
    }
    ...
    if (!contractEntry) errors.push("contract_missing");
      ...
      if (!validate(contract)) errors.push("contract_schema_invalid");
        ...
        if (!primarySource) errors.push("source_file_missing");
          ...
          if (JSON.stringify(sourceIds) !== JSON.stringify(ids)) errors.push("source_contract_case_ids_mismatch");
        }
      }
    }
...
export async function loadExecutionPackage(packagePath: string, options: { requireReady?: boolean } = {}) {
  ...
  const contract = JSON.parse(await zip.file("execution-contract.json")!.async("string")) as { contract_version: "1.0.0"; cases: import("./types.js").ContractCase[] };
  const sourceMapping = JSON.parse(await zip.file("source-mapping.json")!.async("string")) as { cases: Array<{ source_case_id: string; case_id: string; source_sheet: string; source_row: number }> };
  const sourceFiles = new Map<string, Buffer>();
  for (const sourcePath of validation.manifest.source_files) sourceFiles.set(sourcePath, Buffer.from(await zip.file(sourcePath)!.async("uint8array")));
  return {

...
test("Windows runtime lock pins the complete released runtime", async () => {
  const lock = await readJson("packages/testing-runner/release/windows-runtime-lock.json");
  ...
  assert.deepEqual(lock, {
    ...
    node: {
      ...
      windows: {
        x64: {
          download_url: "https://nodejs.org/dist/v22.23.1/node-v22.23.1-win-x64.zip",
          sha256: "7df0bc9375723f4a86b3aa1b7cc73342423d9677a8df4538aca31a049e309c29",
        },
        arm64: {
          download_url: "https://nodejs.org/dist/v22.23.1/node-v22.23.1-win-arm64.zip",
...
test("root and runner manifests pin Playwright without semver ranges", async () => {
  for (const manifestPath of ["package.json", "packages/testing-runner/package.json"]) {
    const manifest = await readJson(manifestPath);
    const version = (manifest.dependencies ?? manifest.devDependencies).playwright;

    assert.equal(version, "1.61.1", manifestPath);
    assert.doesNotMatch(version, /^[~^<>=*]/, manifestPath);
  }
});

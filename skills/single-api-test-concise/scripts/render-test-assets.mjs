...
export function validateReport(data) {
  ...
  for (const sheet of data.sheets) {
    ...
    if (JSON.stringify(sheet.columns) !== JSON.stringify(COLUMNS)) throw new Error(`${sheet.name} 必须使用统一十列`);
    for (const row of sheet.rows) {
      if (!Array.isArray(row.values) || row.values.length !== COLUMNS.length) throw new Error(`${sheet.name} 存在非十列数据`);
      if (row.divider) continue;
      const [id, , , , , , , priority, status] = row.values;
...
async function renderPortableXlsx(data, outputPath) {
  ...
  for (const sheet of sheets) {
    ...
    const widths = sheet.kind === "test_cases" ? [15,18,30,34,34,42,42,10,14,24] : sheet.columns.map((_,i)=>i===0?22:36);
    ...
    const extra = sheet.kind === "test_cases" ? `<conditionalFormatting sqref="A2:J${lastRow}"><cfRule type="expression" dxfId="0" priority="1" stopIfTrue="1"><formula>$I2="不通过"</formula></cfRule><cfRule type="expression" dxfId="1" priority="2" stopIfTrue="1"><formula>$I2="待定"</formula></cfRule></conditionalFormatting><conditionalFormatting sqref="H2:H${lastRow}"><cfRule type="expression" dxfId="2" priority="3"><formula>AND($I2&lt;&gt;"不通过",$I2&lt;&gt;"待定",$H2="P0")</formula></cfRule><cfRule type="expression" dxfId="3" priority="4"><formula>AND($I2&lt;&gt;"不通过",$I2&lt;&gt;"待定",$H2="P1")</formula></cfRule><cfRule type="expression" dxfId="4" priority="5"><formula>AND($I2&lt;&gt;"不通过",$I2&lt;&gt;"待定",$H2="P2")</formula></cfRule></conditionalFormatting><dataValidations count="1"><dataValidation type="list" allowBlank="0" showErrorMessage="1" sqref="${validRows}"><formula1>"未执行,通过,不通过,待定"</formula1></dataValidation></dataValidations>` : "";
  ...
  for (const declared of data.sheets) {
    ...
    if (declared.kind === "test_cases") {
      declared.rows.forEach((row, index) => {
        ...
        const full = sheet.getRange(`A${excelRow}:J${excelRow}`);
        if (row.divider) {
          ...
          if (priority === "P1") sheet.getRange(`H${excelRow}`).format.fill = "#FFF2CC";
          if (priority === "P2") sheet.getRange(`H${excelRow}`).format.fill = "#E2F0D9";
        }
      });
      if (rowCount > 1) {
        const rows = sheet.getRange(`A2:J${rowCount}`);
        rows.conditionalFormats.addCustom('=$I2="不通过"', { fill: "#FCE8E6", font: { color: "#7F1D1D" } });
        rows.conditionalFormats.addCustom('=$I2="待定"', { fill: "#E5E7EB", font: { color: "#374151" } });
      }
    }
  }
  await fs.mkdir(path.dirname(outputPath), { recursive: true });

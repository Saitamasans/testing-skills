...
function Assert-Bundle {
    ...
    if ($runtime.slug -cne 'multi-source-test-audit' -or $runtime.runtime_version -cne '0.1.3' -or $runtime.python_version -cne '3.12.10' -or $runtime.platform -cne 'windows-x64') { throw 'installation_incomplete: runtime manifest identity mismatch' }
    ...
    if ($bundle.slug -cne 'multi-source-test-audit' -or $bundle.runtime_version -cne '0.1.3') { throw 'installation_incomplete: bundle manifest identity mismatch' }
    $expected = @{}
...
try {
    ...
    if ((Get-Content -LiteralPath (Join-Path $script:Root 'VERSION') -Raw).Trim() -cne '0.1.3') { throw 'installation_incomplete: VERSION mismatch' }

...
- 只有需求资料而没有正式测试用例时，不得直接执行。需求文档、需求截图、原型和流程图不能代替正式测试用例；先建议调用 `requirement-test-workbench` 等生成类 Skill 产出 Excel 正式用例，再进入本 Skill。

## 三、准备材料清点

按新用户视角先核对三档材料，再列已具备、缺失和本次不需要的项目：

- **强制资料**：正式测试用例、目标 Web/API 地址、环境性质和执行授权、可执行步骤和预期结果、执行前确认；实际运行 Runner 还必须有 Node.js 20+。
- **条件强制资料**：需要登录时的账号/凭据来源、用例缺少调用细节时的接口文档、写操作所需测试数据和清理方案、Web 稳定定位信息、数据库只读材料、本地应用尚未运行时的前后端源码和启动命令、CI 配置和 Secrets。
- **辅助资料**：需求文档、需求截图、原型、流程图、历史报告和旧证据。已有可访问测试地址时通常不需要前后端源码，除非需要定位控件或排查原因。

需求文档、需求截图、原型和流程图不能代替正式测试用例。只有需求资料而没有正式测试用例时，不得直接执行；先路由到 `requirement-test-workbench` 等生成类 Skill。

...
const SAFE_TREE = /(?:sidebar|main-sidebar|menu-tree|navigation-tree|nav-tree|side-menu|admin-menu)/i;
const GENERIC_NAV = /(?:^|\s)(?:nav|menu)(?:\s|$)/i;
...
export function classifyReadonlyEntry(entry, currentUrl) {
  const label = safeText([entry.text, entry.ariaLabel, entry.title, entry.dataAction, entry.dataMethod, entry.onclick, entry.formaction].filter(Boolean).join(" "));
...
async function discover(page) {
  return page.evaluate(() => [...document.querySelectorAll("a[href], [role=tab], button[data-toggle=tab], button[data-bs-toggle=tab]")].map((el, index) => {
    const container = el.closest(".main-sidebar, .sidebar, .menu-tree, .navigation-tree, .nav-tree, .side-menu, .admin-menu, aside[role=navigation], [role=navigation]");
    const generic = el.closest(".nav, .menu, nav");
    const depth = container ? [...container.querySelectorAll("ul, ol")].filter((node) => node.contains(el)).length : 0;
    return { index, kind: el.matches("[role=tab], button[data-toggle=tab], button[data-bs-toggle=tab]") ? "tab" : "link", text: (el.textContent || "").trim(), ariaLabel: el.getAttribute("aria-label") || "", title: el.getAttribute("title") || "", href: el.href || el.getAttribute("href") || "", disabled: el.matches(":disabled, [aria-disabled=true], .disabled"), inReadonlyContainer: Boolean(container && (/(?:sidebar|main-sidebar|menu-tree|navigation-tree|nav-tree|side-menu|admin-menu)/i.test(container.className || "") || container.matches("aside[role=navigation], [role=navigation]")) && !generic?.classList.contains("nav")), menuDepth: depth, parentMenu: container?.querySelector(".active, .selected, .open, .current")?.textContent?.trim() || container?.getAttribute("aria-label") || null, entryLabel: (el.textContent || el.getAttribute("aria-label") || el.getAttribute("title") || "").trim(), discoveredFrom: location.href, inPagination: Boolean(el.closest(".pagination, [aria-label*=pagination], [class*=pagination]")), onclick: el.getAttribute("onclick") || "", dataMethod: el.getAttribute("data-method") || "", dataAction: el.getAttribute("data-action") || "", dataUrl: el.getAttribute("data-url") || "", dataConfirm: el.getAttribute("data-confirm") || "", formaction: el.getAttribute("formaction") || "", role: el.getAttribute("role") || "", className: el.className?.toString() || "", target: el.getAttribute("target") || "" };
  }));
}
...
export async function traverseReadonlyNavigation({ page, onPage = null, maxPages = 100, settleMs = 150 } = {}) {
  ...
  const queued = new Set([canonicalUrl(page.url(), page.url())]); const visitedUrls = new Set(); const visited = []; const skipped = []; const blocked = []; const paginationCounts = new Map();
  ...
  while (queue.length && visited.length < maxPages) {
    ...
    await settle(page, Math.max(300, settleMs));

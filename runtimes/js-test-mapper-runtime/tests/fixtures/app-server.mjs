...
export async function startDcatReadonlyFixture() {
  ...
  const server = createServer((request, response) => {
    ...
    const shell = (content, title) => "<!doctype html><html><head><title>" + title + "</title><script src=\"/assets/admin.js\"></script></head><body>" +
      "<aside class=\"main-sidebar\"><nav class=\"mt-2\"><ul class=\"nav nav-pills nav-sidebar flex-column\" data-widget=\"treeview\"><li><a class=\"nav-link\" href=\"/admin/dashboard\">Dashboard</a></li><li><a class=\"nav-link\" href=\"/admin/safe-a\">Safe menu A</a></li><li><a class=\"nav-link\" href=\"/admin/broken\">Broken menu</a></li><li><a class=\"nav-link\" href=\"/admin/safe-b\">Safe menu B</a></li><li><a class=\"nav-link\" href=\"/admin/missing-page\">Missing Page</a></li><li><a class=\"nav-link\" href=\"/admin/orders\">Orders</a><ul class=\"nav nav-treeview\"><li><a class=\"nav-link\" href=\"/admin/orders\">Order List</a></li><li><a class=\"nav-link\" href=\"/admin/settings\">Settings</a></li></ul></li><li><a class=\"nav-link\" href=\"/admin/orders/create\">Create Order</a></li><li><a class=\"nav-link\" href=\"/admin/cache/clear\">Clear Cache</a></li><li><a class=\"nav-link\" href=\"/admin/password/reset\">Reset Password</a></li><li><a class=\"nav-link\" data-url=\"/admin/data/sync\" href=\"/admin/data/sync\">Sync Data</a></li><li><a class=\"nav-link\" href=\"/admin/refund\">Refund</a></li><li><a class=\"nav-link\" href=\"/admin/orders/cancel\">Cancel Order</a></li><li><a class=\"nav-link\" href=\"/admin/regenerate\">Regenerate</a></li><li><a class=\"nav-link\" href=\"/admin/tasks/run\">Run Task</a></li><li><a class=\"nav-link\" href=\"/admin/orders/export\">Export</a></li></ul></nav></aside>" +
      "<main><nav class=\"breadcrumb\"><a href=\"/admin/dashboard\">Home</a><span class=\"breadcrumb-item\">" + title + "</span></nav>" + content + "</main></body></html>";
    if (pathname === "/admin/broken") { setTimeout(() => send("timeout"), 6000); return; }
    ...
    if (pathname === "/admin/dashboard") return send(shell("<h1>Dashboard</h1><p>Overview</p><script>setTimeout(() => { const a=document.createElement(\"a\"); a.href=\"/admin/delayed\"; a.textContent=\"Delayed Report\"; document.querySelector(\".main-sidebar\").append(a); }, 320);</script>", "Dashboard"));
    if (pathname === "/admin/safe-a") return send(shell("<h1>Safe A</h1>", "Safe A"));
    if (pathname === "/admin/safe-b") return send(shell("<h1>Safe B</h1>", "Safe B"));
    if (pathname === "/admin/delayed") return send(shell("<h1>Delayed Report</h1>", "Delayed Report"));
    if (pathname === "/admin/orders/1/history") return send(shell("<h1>History detail</h1>", "History detail"));
    if (pathname === "/admin/orders") return send(shell("<h1>Orders</h1><form><input name=\"q\" placeholder=\"Search orders\"><select name=\"status\"><option>All</option></select></form><table><thead><tr><th>ID</th><th>Status</th><th>Total</th></tr></thead><tbody><tr><td><a href=\"/admin/orders/1\">View</a></td><td>Pending</td><td>10</td></tr></tbody></table><div class=\"pagination\"><a href=\"/admin/orders?page=2\">2</a><a href=\"/admin/orders?page=2\">Next</a></div><div class=\"tabs\"><a class=\"nav-link\" data-toggle=\"pill\" href=\"#history\" role=\"tab\">History</a><a class=\"nav-link\" data-toggle=\"pill\" href=\"#notes\" role=\"tab\">Notes</a></div><section id=\"history\" hidden></section><script>document.querySelectorAll(\"[data-toggle=\\\"pill\\\"]\").forEach(tab => tab.addEventListener(\"click\", () => { if(tab.getAttribute(\"href\")==\"#history\") document.querySelector(\"#history\").innerHTML=\'<a href=\"/admin/orders/1/history\">History detail</a>\'; }));</script><a href=\"/admin/settings/unknown\">Open</a>", "Orders"));

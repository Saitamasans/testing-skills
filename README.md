...
面向中文功能测试用户的 10 个 Agent Skill，覆盖需求澄清、测试设计、灰盒增强用例、用例审计、正式服验证、人工 Web UI 用例的可执行化编译，以及已有 Web/API 用例的自动执行和证据回填。`enhanced-graybox-test-case-generation` 按用户产品规划称为“第十一个 Skill”；当前仓库没有第十个 Skill，因此这里按实际可用包数量统计，不虚构缺失编号。

[选择 Skill](#skills) · [安装](#install) · [第 1–7 个 Skill 使用指南](#usage-guides) · [第十一个 Skill 专项指南](#graybox-guide) · [第 9 个 Skill 专项指南](#compiler-guide) · [第 8 个 Skill 专项指南](#execution-guide) · [输出文件](#outputs)
...
| 增强版-灰盒测试用例生成<br>`enhanced-graybox-test-case-generation` | 结合需求、前后端代码、Admin、接口文档、数据库结构和代码 Diff，完成需求门禁、实现影响分析、灰盒测试设计、回归范围识别和正式测试用例生成。 | 安装器待下一版本发布；本地源码与通用安装脚本可用。 |
...
第十一个 Skill 尚未进入任何不可变 Release，不提供或伪造 Release 下载按钮。当前可从本仓库源码使用 `scripts/install.ps1 -Skill 'enhanced-graybox-test-case-generation' -SourceDirectory .` 安装；正式按钮需在下一版本发布并上传对应资产后再启用。
...
2. 安装完成后重启 Codex。
3. 先调用第九个 Skill，把人工测试用例编译成 `*.execution-package.zip`；再把该 ZIP 交给第八个 Skill 执行。
...
- CC Switch：打开 Skills 管理页，读取各包的 `SKILL.md` 名称和 description，并分别管理当前仓库的 10 个 Skill。
...
适合根据 PRD、用户故事或需求变更做需求评审、测试设计、正式用例或回归分析。只有需求资料也可以使用，不要求提供代码；输出是标准需求驱动测试用例。
...
<a id="graybox-guide"></a>

## 增强版-灰盒测试用例生成 即第十一个 Skill 专项指南

对应 Package：`enhanced-graybox-test-case-generation`。

### 适合使用

- 结合需求和项目代码生成测试用例；
- 结合前端、后端、Admin 和接口文档补齐场景；
- 根据新需求和本次 Diff 更新用例与回归范围；
- 根据客户端真实传参、后端校验、数据库、Redis、Queue 或 WebSocket 生成灰盒增强用例。

### 不适合使用

只有需求文档时使用 `requirement-test-workbench`；只需需求澄清时使用 `requirement-clarification-test`；只有接口文档时使用单接口或多接口 Skill；只有代码审计目标、只有现有用例评审、正式服轻量验证或自动执行已有用例时，使用对应专用 Skill。

### 最少准备

一份需求资料，加至少一种与当前需求相关的实现资料。推荐补充产品答复、原型、前后端/Admin 代码、接口文档、数据库结构、缓存/异步实现、本次 Diff、已有用例和相关历史 Bug；资料不要求全部齐全，缺失层会标记为证据缺口。

### 关键门禁与输出

需求决定预期，代码只说明当前实现。L0 一句话需求即使附完整代码，也必须先关闭 P0，不能直接生成正式用例。只有需求文档时，本 Skill 简洁路由到需求测试工作台，不扫描代码，也不把缺代码当成阻塞。

本 Skill 至少需要“需求 + 一种相关实现资料”。它以正式测试用例为第一核心交付物，面向测试人员手工执行；代码分析只服务于提高用例准确度、细节、边界、权限、数据闭环和回归判断，不是代码审计，也不追求覆盖全部分支。

默认交付输入准入、需求基线、高价值差异、一套严格 10 列正式用例、核心用例说明、回归范围、待确认、待动态验证和未覆盖风险，并从同一报告生成 Excel 与离线 HTML。实现证据可帮助识别多端不一致、对象级权限、缓存/异步时点、牵连旧功能和历史兼容；每项高价值发现必须进入正式用例、待产品确认或待动态验证。

### 调用示例

```text
调用 `enhanced-graybox-test-case-generation`：结合这份房间管理需求、Android、后端、Admin 和接口文档生成完整灰盒测试用例。
```

```text
调用 `enhanced-graybox-test-case-generation`：根据这个新需求和本次 PR Diff，补充受影响用例和回归范围。
```

```text
调用 `enhanced-graybox-test-case-generation`：结合客户端真实传参、后端权限校验、数据库字段和 Redis 缓存生成灰盒测试用例。
```

```text
调用 `enhanced-graybox-test-case-generation`：当前只有一句需求和完整代码，请先按需求门禁澄清，不要直接生成正式用例。
```

```text
调用 `enhanced-graybox-test-case-generation`：结合需求、Android、后端、Admin 和接口文档，生成供测试人员手工执行的正式测试用例，并识别受影响回归范围。代码分析只服务于用例生成，不输出深度代码审计报告。
```

<a id="compiler-guide"></a>

## 9. 测试用例可执行化编译 即 第 9 个 Skill 专项指南

对应 Package：`test-case-execution-compiler`。

第九个 Skill 把已有标准十列、十一列或已确认字段映射的 Web UI 人工用例编译成一个 `*.execution-package.zip`。人工测试用例是业务事实来源；编译过程不新增正式业务用例、不修改原始 Excel、不访问目标网页，也不启动 Playwright 或 Chromium。

公开工作流固定为：

```text
人工测试用例.xlsx -> test-case-execution-compiler -> *.execution-package.zip -> web-api-test-execution-evidence
```

ZIP 内包含原始用例、Execution Contract 1.0.0、映射、readiness、待确认项和 package manifest。用户工作目录只产生这一个 ZIP；若存在必须确认的问题，包状态为 `NOT_READY`，补齐后重新编译，不能直接修改旧 ZIP。原用例变化后旧包会因 SHA-256 不一致而失效。

调用示例：

```text
调用 `test-case-execution-compiler`：请把附件中的已有 Web UI 人工测试用例编译成 Execution Package；不要修改原始用例，不要打开网页，未知状态写入待确认项。
```
...
已有 READY `*.execution-package.zip`，需要绑定真实页面、审批、自动执行、收集证据、回填 Excel/HTML，或在本地和 CI 中校验执行产物时使用。
...
- **正式执行包：** 第九个 Skill 输出的 READY `*.execution-package.zip`；第八个 Skill 直接收到普通 Excel 或 JSON 时默认返回 `code=execution_contract_required`，请先调用 `test-case-execution-compiler`。
...
- 包的 ZIP 路径、内部 SHA、源文件 SHA、用例数量与 ID、READY 状态和契约 Schema 必须全部通过校验。
...
```text
调用 `web-api-test-execution-evidence`：请执行附件中的 READY Execution Package；目标是测试环境 `https://example.test`，凭据从环境变量读取。先做真实页面探测和语义绑定，展示最终 manifest 预览并等待确认；确认后再运行并回填报告和证据。
```
...
`enhanced-graybox-test-case-generation` 通过需求门禁并产出正式用例时，默认生成 `.xlsx` 和 `.html`；Excel 主 Sheet 固定为“正式测试用例”。它与 `requirement-test-workbench` 共用 `requirement-test-case-v1` 合同和黑体 `SimHei` 样式，但两个安装包仍各自自包含。
...
统一十列为：用例 ID、所属模块、用例标题、验证功能点、前置条件、测试步骤、预期结果、优先级、执行结果（通过 / 不通过 / 未执行）、备注。旧 8 列和旧 11 列只作为输入兼容格式；重新生成时补齐缺失字段、移除旧“实际结果”列、重新编号并把执行结果置为“未执行”。
...
请不要直接编辑自动生成的 `skills/*/SKILL.md`；应修改对应源文件后运行 builder。原 7 个源文件与第十一个 Skill 的 `增强版-灰盒测试用例生成_Skill.md` 在根目录，第 8、9 个源文件分别在 `skill-sources/web-api-test-execution-evidence/` 和 `skill-sources/test-case-execution-compiler/`。
